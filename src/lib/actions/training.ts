...
import { all, batch, get, scalar } from "@/lib/db";
...
export async function createExercise(form: FormData): Promise<ActionResult<{ id: string }>> {
  ...
  const clash = await get<{ id: string }>("SELECT id FROM exercises WHERE name = ?", [v.name]);
  if (clash) return fail(`An exercise called "${v.name}" already exists.`);

  const newId = await insert("exercises", {
      name: v.name,
      category: v.category,
      modality: v.modality,
...
export async function updateExercise(form: FormData): Promise<ActionResult> {
  ...
  await update("exercises", exerciseId, {
        ...rest,
...
export async function archiveExercise(exerciseId: string): Promise<ActionResult> {
  await update("exercises", exerciseId, { archived: 1 });
  refresh("/body/strength");
  return ok();
}
...
export async function createWorkout(form: FormData): Promise<ActionResult<{ id: string }>> {
  ...
  if (!parsed.ok) return parsed.result;
  const v = parsed.value;
  const newId = await insert("workouts", {
      name: v.name,
      type: v.type,
      focus: v.focus ?? null,
...
export async function updateWorkout(form: FormData): Promise<ActionResult> {
  ...
  await update("workouts", workoutId, rest);
...
export async function deleteWorkout(workoutId: string): Promise<ActionResult> {
  await remove("workouts", workoutId);
...
export async function updateWorkoutExercise(form: FormData): Promise<ActionResult> {
  ...
  const row = await get<{ workout_id: string }>("SELECT workout_id FROM workout_exercises WHERE id = ?", [
      rowId,
    ]);
  await update("workout_exercises", rowId, rx);
...
export async function removeWorkoutExercise(rowId: string): Promise<ActionResult> {
  ...
  await remove("workout_exercises", rowId);
  if (row) refresh(`/body/training/library/${row.workout_id}`);
  return ok();
}

export async function moveWorkoutExercise(rowId: string, direction: string): Promise<ActionResult> {
  const row = await get<WorkoutExercise>("SELECT * FROM workout_exercises WHERE id = ?", [rowId]);
  if (!row) return fail("Exercise not found in this workout.");
  const neighbour = await get<WorkoutExercise>(
      direction === "up"
  ...
  await update("workout_exercises", row.id, { sort_order: neighbour.sort_order });
  await update("workout_exercises", neighbour.id, { sort_order: row.sort_order });
...
export async function scheduleSession(form: FormData): Promise<ActionResult<{ id: string }>> {
  ...
  if (!workout_id) {
    ...
    const blankId = await insert("workout_sessions", {
          workout_id: null,
          date,
          name,
          type: "STRENGTH",
  ...
  const detail = await workoutDetail(workout_id);
  ...
  for (const row of detail.exercises) {
    const last = await lastPerformance(row.exercise_id, date);
    const progression = computeProgression(row.exercise, row, last);
    await insert("session_exercises", {
...
export async function startSession(sessionId: string): Promise<ActionResult> {
  const session = await get<{ date: string; status: string }>(
  ...
  await update("workout_sessions", sessionId, { status: "IN_PROGRESS", started_at: nowIso() });
  await recomputeDayScore(session.date);
  refresh(`/body/training/${sessionId}`);
  return ok();
}
...
export async function completeSession(form: FormData): Promise<ActionResult<{ records: DetectedRecord[] }>> {
  ...
  const session = await get<{ date: string; started_at: string | null }>(
  ...
  await update("workout_sessions", v.id, {
        status: "COMPLETED",
        ended_at: endedAt,
  ...
  const records = await recordSessionPRs(v.id, session.date);
  await recomputeDayScore(session.date);
...
export async function setSessionStatus(sessionId: string, status: string): Promise<ActionResult> {
  ...
  if (!parsed.ok) return parsed.result;
  const session = await get<{ date: string }>("SELECT date FROM workout_sessions WHERE id = ?", [
      sessionId,
    ]);
  if (!session) return fail("Session not found.");
  await update("workout_sessions", sessionId, {
...
export async function deleteSession(sessionId: string): Promise<ActionResult> {
  ...
  if (session) await recomputeDayScore(session.date);
  refresh();
  return ok();
}
...
export async function addSessionExercise(form: FormData): Promise<ActionResult> {
  ...
  const last = await lastPerformance(exercise_id, session.date, session_id);
...
export async function updateSessionExercise(form: FormData): Promise<ActionResult> {
  ...
  if (!parsed.ok) return parsed.result;
  const { id: rowId, ...rx } = parsed.value;
  const row = await get<{ session_id: string }>("SELECT session_id FROM session_exercises WHERE id = ?", [
      rowId,
    ]);
  if (!row) return fail("Exercise not found in this session.");
  await update("session_exercises", rowId, { ...rx, target_source: "MANUAL" });
...
export async function removeSessionExercise(rowId: string): Promise<ActionResult> {
  ...
  await remove("session_exercises", rowId);
...
export async function recomputeSessionTargets(sessionId: string): Promise<ActionResult> {
  ...
  for (const row of rows) {
    ...
    await update("session_exercises", row.id, {
...
export async function logSet(form: FormData): Promise<ActionResult<{ id: string }>> {
  ...
  const sx = await get<SessionExercise>("SELECT * FROM session_exercises WHERE id = ?", [
  ...
  const setId = await insert("workout_sets", {
  ...
  if (session.status === "PLANNED") {
    await update("workout_sessions", sx.session_id, { status: "IN_PROGRESS", started_at: nowIso() });
  }
...
export async function deleteSet(setId: string): Promise<ActionResult> {
  const row = await get<{ session_id: string; session_exercise_id: string; date: string }>(
      "SELECT session_id, session_exercise_id, date FROM workout_sets WHERE id = ?",
      [setId],
    );
  if (!row) return fail("Set not found.");
  await remove("workout_sets", setId);
  ...
  await batch(
    remaining.map((r, i) => ({
      sql: "UPDATE workout_sets SET set_index = ? WHERE id = ?",
      params: [i + 1, r.id],
    })),
  );
...
export async function logTargetSet(
  ...
  if (!sx) return fail("Exercise not found in this session.");
  const exercise = await get<Exercise>("SELECT * FROM exercises WHERE id = ?", [sx.exercise_id]);
  if (!exercise) return fail("Exercise not found.");
