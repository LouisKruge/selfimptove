...
export async function logRun(form: FormData): Promise<ActionResult<{ id: string; records: DetectedRecord[] }>> {
  ...
  const records = await recordRunPRs(runId, v.date);
...
export async function updateRun(form: FormData): Promise<ActionResult> {
  ...
  const existing = await get<{ distance_m: number | null; duration_sec: number | null; date: string }>(
  ...
  await recomputeDayScore(rest.date ?? existing.date);
  refresh("/body/running", `/body/running/${runId}`);
  return ok();
}

export async function deleteRun(runId: string): Promise<ActionResult> {
  const row = await get<{ date: string }>("SELECT date FROM runs WHERE id = ?", [runId]);
  await run("DELETE FROM personal_records WHERE domain = 'RUN' AND source_id = ?", [runId]);
  await remove("runs", runId);
...
export async function updateRunInterval(form: FormData): Promise<ActionResult> {
  ...
  if (!parsed.ok) return parsed.result;
  const v = parsed.value;
  const row = await get<{ run_id: string; distance_m: number | null; duration_sec: number | null }>(
  ...
  await update("run_intervals", v.id, {
        distance_m: distance,
...
export async function deleteRunInterval(intervalId: string): Promise<ActionResult> {
  const row = await get<{ run_id: string }>("SELECT run_id FROM run_intervals WHERE id = ?", [intervalId]);
  await remove("run_intervals", intervalId);
...
export async function updateHyroxStation(form: FormData): Promise<ActionResult> {
  ...
  const row = await get<{ hyrox_session_id: string; distance_m: number | null }>(
  ...
  const distance = v.distance_m ?? row.distance_m;
  await update("hyrox_stations", v.id, {
...
export async function deleteHyroxStation(stationId: string): Promise<ActionResult> {
  const row = await get<{ hyrox_session_id: string }>(
      "SELECT hyrox_session_id FROM hyrox_stations WHERE id = ?",
      [stationId],
    );
  await remove("hyrox_stations", stationId);
  if (row) {
    await recalcHyroxTotals(row.hyrox_session_id);
    refresh(`/body/hyrox/${row.hyrox_session_id}`);
  }
...
export async function recalcHyroxTotals(hyroxSessionId: string): Promise<ActionResult> {
  const stations = await all<{ station: string; duration_sec: number | null; transition_sec: number | null }>(
  ...
  await update("hyrox_sessions", hyroxSessionId, {
...
export async function completeHyroxSession(
  ...
  const row = await get<{ date: string }>("SELECT date FROM hyrox_sessions WHERE id = ?", [
      hyroxSessionId,
    ]);
  if (!row) return fail("HYROX session not found.");
  await recalcHyroxTotals(hyroxSessionId);
  const records = await recordHyroxPRs(hyroxSessionId, row.date);
...
export async function deleteHyroxSession(hyroxSessionId: string): Promise<ActionResult> {
  ...
  await run("DELETE FROM personal_records WHERE domain = 'HYROX' AND source_id = ?", [
    hyroxSessionId,
  ]);
  await remove("hyrox_sessions", hyroxSessionId);
  if (row) await recomputeDayScore(row.date);
  refresh("/body/hyrox");
  return ok();
}
...
async function ensureNutritionLog(date: string): Promise<NutritionLog> {
  const existing = await get<NutritionLog>("SELECT * FROM nutrition_logs WHERE date = ?", [date]);
  if (existing) return existing;
  await insert("nutrition_logs", { date });
  return (await get<NutritionLog>("SELECT * FROM nutrition_logs WHERE date = ?", [date]))!;
}

async function recalcNutritionLog(date: string) {
  const row = await get<{
      calories: number;
      protein_g: number;
      carbs_g: number;
  ...
  const log = await ensureNutritionLog(date);
  await update("nutrition_logs", log.id, {
...
export async function logMeal(form: FormData): Promise<ActionResult> {
  ...
  const log = await ensureNutritionLog(v.date);
  await insert("meals", {
  ...
  if (v.save_preset) {
    const existing = await get<{ id: string; use_count: number }>(
          "SELECT id, use_count FROM meal_presets WHERE name = ?",
          [v.name],
        );
    if (existing) await update("meal_presets", existing.id, { ...values, use_count: existing.use_count + 1 });
    else await insert("meal_presets", { name: v.name, slot: v.slot, ...values, use_count: 1 });
  }

  await recalcNutritionLog(v.date);
  await recomputeDayScore(v.date);
...
export async function logMealPreset(presetId: string, date: string): Promise<ActionResult> {
  ...
  await update("meal_presets", presetId, { use_count: preset.use_count + 1 });
  return await logMeal(form);
}

export async function deleteMeal(mealId: string): Promise<ActionResult> {
  const row = await get<{ date: string }>("SELECT date FROM meals WHERE id = ?", [mealId]);
  await remove("meals", mealId);
  if (row) {
    await recalcNutritionLog(row.date);
    await recomputeDayScore(row.date);
  }
  refresh("/body/nutrition");
  return ok();
...
export async function logWater(form: FormData): Promise<ActionResult> {
  ...
  const log = await ensureNutritionLog(v.date);
  await update("nutrition_logs", log.id, { water_ml: (log.water_ml ?? 0) + v.water_ml });
  refresh("/body/nutrition");
  return ok();
}
...
export async function setNutritionTarget(form: FormData): Promise<ActionResult> {
  ...
  const existing = await get<{ id: string }>("SELECT id FROM nutrition_targets WHERE effective_from = ?", [
      v.effective_from,
    ]);
  const values = {
  ...
  if (existing) await update("nutrition_targets", existing.id, values);
  else await insert("nutrition_targets", values);
...
export async function logMeasurement(form: FormData): Promise<ActionResult> {
  ...
  const existing = await get<{ id: string }>("SELECT id FROM body_measurements WHERE date = ?", [date]);
  if (existing) await update("body_measurements", existing.id, rest);
  else await insert("body_measurements", { date, ...rest });

  await recomputeDayScore(date);
  refresh("/body/measurements");
  return ok();
}

export async function deleteMeasurement(measurementId: string): Promise<ActionResult> {
  await remove("body_measurements", measurementId);
  refresh("/body/measurements");
  return ok();
}
...
export async function logRecovery(form: FormData): Promise<ActionResult> {
  ...
  const existing = await get<{ id: string }>("SELECT id FROM recovery_logs WHERE date = ?", [date]);
  const values = { ...rest, is_rest_day: is_rest_day ? 1 : 0 };
  if (existing) await update("recovery_logs", existing.id, values);
  else await insert("recovery_logs", { date, ...values });
...
export async function quickLogWeight(form: FormData): Promise<ActionResult> {
  ...
  return await logMeasurement(next);
}
