...
export async function createHabit(form: FormData): Promise<ActionResult> {
  ...
  const active = scalar("SELECT COUNT(*) AS v FROM habits WHERE active = 1");
  if (active >= 8) {
    return fail(
...
export async function toggleHabit(habitId: string, date: string): Promise<ActionResult> {
  ...
  const existing = get<{ id: string; done: number }>(
    "SELECT id, done FROM habit_logs WHERE habit_id = ? AND date = ?",
    [habitId, day],
  );
  if (existing) {
    if (existing.done === 1) remove("habit_logs", existing.id);
    else update("habit_logs", existing.id, { done: 1 });
  } else {
    insert("habit_logs", { habit_id: habitId, date: day, done: 1 });
  }
...
export async function resolvePromise(promiseId: string, status: string): Promise<ActionResult> {
  ...
  const row = get<{ date: string }>("SELECT date FROM promises WHERE id = ?", [promiseId]);
  if (!row) return fail("Promise not found.");

  update("promises", promiseId, {
...
export async function createDecision(form: FormData): Promise<ActionResult<{ id: string; level: string }>> {
  ...
  const verdict = classifyDecision({
    ...
    majorSpendCents: getSettingNumber("major_spend_cents", 500_000),
  });

  const until = coolingUntil(verdict.level);
  const decisionId = insert("decisions", {
    title: v.title,
...
export async function deleteDecisionOption(optionId: string): Promise<ActionResult> {
  const row = get<{ decision_id: string }>(
    "SELECT decision_id FROM decision_options WHERE id = ?",
    [optionId],
  );
  remove("decision_options", optionId);
...
export async function decide(form: FormData): Promise<ActionResult> {
  ...
  const row = get<{ cooling_until: string | null; status: string }>(
  ...
  update("decisions", v.id, {
    decision: v.decision,
    status: "DECIDED",
    decided_at: nowIso(),
  });
  if (v.option_id) {
    update("decision_options", v.option_id, { chosen: 1 });
  }
...
export async function upsertSkill(form: FormData): Promise<ActionResult> {
  ...
  if (v.id) update("skills", v.id, values);
  else {
    const clash = get<{ id: string }>("SELECT id FROM skills WHERE name = ?", [v.name]);
...
export async function markLearningApplied(form: FormData): Promise<ActionResult> {
  ...
  if (!parsed.ok) return parsed.result;
  const row = get<{ date: string }>("SELECT date FROM learning_items WHERE id = ?", [
    parsed.value.id,
  ]);
  if (!row) return fail("Learning item not found.");

  update("learning_items", parsed.value.id, {
    applied: 1,
    applied_at: nowIso(),
...
export async function deleteLearningItem(itemId: string): Promise<ActionResult> {
  const row = get<{ date: string }>("SELECT date FROM learning_items WHERE id = ?", [itemId]);
  remove("learning_items", itemId);
  if (row) recomputeDayScore(row.date);
  refreshPaths(["/learning"]);
  return ok();
}
...
export async function setIdeaStage(ideaId: string, stage: string): Promise<ActionResult> {
  ...
  update("ideas", ideaId, { stage: parsed.value.stage });
  refreshPaths(["/ideas"]);
  refreshPaths([`/ideas/${ideaId}`]);
  return ok();
}
