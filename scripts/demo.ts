...
export function generateDemo(db: Database): string {
  ...
  const exerciseId = (name: string) =>
    one<{ id: string }>("SELECT id FROM exercises WHERE name = ?", [name])?.id;
  const workout = (name: string) =>
    one<{ id: string; type: string }>("SELECT id, type FROM workouts WHERE name = ?", [name]);
  ...
  db.transaction(() => {
    ...
    insert("scheduled_cash_items", { name: "Rent", direction: "OUT", amount_cents: 12_500_00, cadence: "MONTHLY", day_of_month: 1, category: "Housing", active: 1 });
    insert("scheduled_cash_items", { name: "Vehicle finance", direction: "OUT", amount_cents: 5_400_00, cadence: "MONTHLY", day_of_month: 1, category: "Debt", active: 1 });
    insert("scheduled_cash_items", { name: "Credit card minimum", direction: "OUT", amount_cents: 3_200_00, cadence: "MONTHLY", day_of_month: 28, category: "Debt", debt_id: debtId, active: 1 });
    insert("scheduled_cash_items", { name: "Groceries", direction: "OUT", amount_cents: 1_800_00, cadence: "WEEKLY", day_of_week: 6, category: "Food", active: 1 });
    insert("scheduled_cash_items", { name: "Gym", direction: "OUT", amount_cents: 899_00, cadence: "MONTHLY", day_of_month: 3, category: "Health", active: 1 });
    insert("scheduled_cash_items", { name: "Salary", direction: "IN", amount_cents: 32_000_00, cadence: "MONTHLY", day_of_month: 25, category: "Income", active: 1 });
    ...
    insert("assets", { name: "Vehicle (demo)", kind: "VEHICLE", value_cents: 176_000_00 });

    /* ----------------------------------------------------------- BUSINESS */
    const business = one<{ id: string }>("SELECT id FROM businesses LIMIT 1");
    if (business) {
      db.prepare(
        "UPDATE businesses SET target_customer = ?, offer = ?, avg_deal_cents = ?, stage = ?, model = ?, updated_at = ? WHERE id = ?",
      ).run(
        "Owner-run trades businesses in Gauteng doing R200k–R800k/month who lose quotes to slow follow-up.",
    ...
    const milestones = many<{ id: string; sort_order: number }>(
    ...
    milestones.forEach((m, i) => {
      ...
      db.prepare("UPDATE milestones SET status = ?, completed_at = ?, updated_at = ? WHERE id = ?").run(
        status,
    ...
    const customerCount = (one<{ v: number }>("SELECT COUNT(*) AS v FROM customers")?.v ?? 0);
    const revenueTotal = (one<{ v: number }>("SELECT COALESCE(SUM(amount_cents),0) AS v FROM revenue_entries")?.v ?? 0);
    ...
    for (const [name, value] of kpiUpdates) {
      db.prepare(
        "UPDATE mission_kpis SET current_value = ?, updated_at = ? WHERE mission_id = ? AND name = ?",
      ).run(value, now(), mission?.id ?? "", name);
    }

    /* Backdate the season and mission so elapsed time reflects the demo window. */
    db.prepare("UPDATE seasons SET start_date = ?, updated_at = ? WHERE status = 'ACTIVE'").run(
      start,
      now(),
    );
    if (mission) {
      db.prepare("UPDATE missions SET start_date = ?, end_date = ?, current_value = ?, updated_at = ? WHERE id = ?").run(
        start,
        addDays(start, 89),
    ...
    /* ------------------------------------------------------------ TRAINING */
    const templates = ["PUSH A", "PULL A", "LEGS A", "UPPER", "LOWER"]
      .map(workout)
    ...
    for (const day of days) {
      ...
      if (isLiftDay) {
        ...
        const name = one<{ name: string }>("SELECT name FROM workouts WHERE id = ?", [template.id])!.name;
        ...
        for (const [i, row] of rows.entries()) {
          const exName = one<{ name: string; increment_kg: number }>(
            "SELECT name, increment_kg FROM exercises WHERE id = ?",
            [row.exercise_id],
          );
          if (!exName) continue;

          const sxId = insert("session_exercises", {
      ...
      if (isRunDay && !chance(0.15)) {
        ...
        const runSessionId = insert("workout_sessions", {
          date: day,
      ...
      if (isHyroxDay && !chance(0.2)) {
        const w = workout("HYROX CONDITIONING");
      ...
      if (!chance(0.1)) {
        ...
        const logId = insert("nutrition_logs", {
          date: day,
          calories,
          protein_g: protein,
      ...
      if (!isWeekend && chance(0.6)) {
        ...
        const skill = one<{ id: string }>("SELECT id FROM skills ORDER BY RANDOM() LIMIT 1");
        insert("learning_items", {
    ...
    let seq = 1;
    sim.forEach(([station, seconds, extra], i) => {
    ...
    const totals = one<{ run: number; work: number; trans: number }>(
      `SELECT
    ...
    db.prepare(
      "UPDATE hyrox_sessions SET run_total_sec = ?, station_total_sec = ?, transition_sec = ?, total_sec = ?, updated_at = ? WHERE id = ?",
    ).run(
      totals?.run ?? 0,
      totals?.work ?? 0,
      totals?.trans ?? 0,
    ...
    insert("ideas", { title: "Weekly newsletter for trades owners", stage: "CAPTURE" });
